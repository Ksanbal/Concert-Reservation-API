...
export const QueueMockRepository = {
  findLastWorkingQueue: jest.fn(() => {
    return mockData.find((queue) => queue.status === QueueStatusEnum.WORKING);
  }),
  ...
  findByToken: jest.fn((token: string) => {
    return mockData.find((queue) => queue.token === token);
  }),
  ...
  workingQueueCount: jest.fn(() => {
    return mockData.filter((queue) => queue.status === QueueStatusEnum.WORKING)
      .length;
  }),
  ...
  findExpiredQueues: jest.fn(() => {
    return mockData.filter((queue) => queue.status === QueueStatusEnum.EXPIRED);
  }),
};
